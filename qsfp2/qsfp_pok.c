...
//		long long int i2c_chip[toml_array_nelem(arr)];
		...
		int mod_ind[] = {8,22,23}; // temporary   

		typedef struct pok_bit_s
		{
			uint8_t byte_num;
			uint8_t bit_num;
		} pok_bit_t;

		// positions of POK bits in the 0x10 register
		pok_bit_t pok_bit[8] = 
		{
			{2, 5},
			{2, 0},
		...
		{
			i2c_read_qsfp2 (i2c_fd_bus, exp0, er[0]);
			// extract POK bits
			uint8_t pok = 0;
			for (int i = 0; i < 8; i++)
			{
				pok |= (((er[0][pok_bit[i].byte_num] >> pok_bit[i].bit_num) & 1) << i);
			}
			if (pok != 0xff) 
			{
				printf ("POK = %02x\n", pok);
...
//				if (0)
				...
				// read RX amplitudes
				uint8_t reg_238, reg_239, reg_225, ampl;
